...
test.describe("search for rides", () => {
    ...
    test("guest can search for a route and see results page", async ({ page }) => {
        const inputs = page.getByRole("textbox");

        await inputs.nth(0).fill("Bratislava");
        await page.getByRole("option", { name: /Bratislava/i }).first().click();
        ...
        await expect(page).toHaveURL(/\/rides\?.*startCity=Bratislava.*destCity=Nitra/i);
