    ...
    describe("Car Management", () => {
        ...
        it("updates car status via PATCH /cars/:id/status", async () => {
            ...
            const createResponse = await authenticatedRequest(
                ...
                jsonRequest({
                    modelId,
                    spz: `KE${crypto.randomUUID().slice(0, 5).toUpperCase()}`,
                    countryCode: "SK",
                    color: "WHITE",
                    seatsTotal: 4,
                })
            );
            const car = (await createResponse.json()) as {
                id: string;
                isActive: boolean;
            };
            ...
            const updatedCar = (await patchResponse.json()) as {
                isActive: boolean;
            };
        ...
        it("returns 404 when trying to update someone else's car status", async () => {
            ...
            const createResponse = await authenticatedRequest(
                ...
                jsonRequest({
                    modelId,
                    spz: `PO${crypto.randomUUID().slice(0, 5).toUpperCase()}`,
                    countryCode: "SK",
                    color: "BLACK",
                    seatsTotal: 4,
                })
        ...
        it("soft-deletes a car via DELETE /cars/:id", async () => {
            ...
            const createResponse = await authenticatedRequest(
                ...
                jsonRequest({
                    modelId,
                    spz: `ZA${crypto.randomUUID().slice(0, 5).toUpperCase()}`,
                    countryCode: "SK",
                    color: "RED",
                    seatsTotal: 4,
                })
        ...
        it("returns 409 CarInUse when deleting a car assigned to an active ride", async () => {
            ...
            const createResponse = await authenticatedRequest(
                ...
                jsonRequest({
                    modelId,
                    spz: `BB${crypto.randomUUID().slice(0, 5).toUpperCase()}`,
                    countryCode: "SK",
                    color: "BLUE",
                    seatsTotal: 4,
                })
