...
describe("ReviewRoutes", () => {
    describe("Authorization & Onboarding Guards", () => {
        it("returns 401 UNAUTHORIZED for POST /reviews without a session", async () => {
            const response = await apiRequest("/reviews", jsonRequest({
        ...
        it("returns 403 ONBOARDING_REQUIRED for POST /reviews with a non-onboarded user", async () => {
            ...
            const response = await authenticatedRequest("/reviews", cookie, jsonRequest({
                rideId: crypto.randomUUID(),
                subjectId: crypto.randomUUID(),
                rating: 5,
            }));
        ...
        it("returns 401 UNAUTHORIZED for GET /reviews/users/:id without a session", async () => {
            const response = await apiRequest(`/reviews/users/${crypto.randomUUID()}`);
            expect(response.status).toBe(401);
        });
    ...
    describe("Review Creation Validation (Negative Tests)", () => {
        it("returns 400 SelfReviewNotAllowed when reviewing yourself", async () => {
            const { driver, driverCookie, rideId } = await setupCompletedRideWithPassenger();

            const response = await authenticatedRequest("/reviews", driverCookie, jsonRequest({
                rideId,
        ...
        it("returns 404 RideNotFound for a random rideId", async () => {
            const { driverCookie, passenger } = await setupCompletedRideWithPassenger();
        ...
        it("returns 403 AuthorNotInRide when a random user tries to write a review", async () => {
            ...
            const response = await authenticatedRequest("/reviews", stranger.cookie, jsonRequest({
                rideId,
                subjectId: driver.id,
                rating: 4
            }));
        ...
        it("returns 400 SubjectNotInRide when reviewing a random user not in the ride", async () => {
            const { driverCookie, rideId } = await setupCompletedRideWithPassenger();
            const stranger = await createSignedInUser(); // Not in the ride
        ...
        it("returns 400 RideNotCompleted when ride is still PLANNED", async () => {
            ...
            const response = await authenticatedRequest("/reviews", passengerCookie!, jsonRequest({
                rideId,
                subjectId: driver.id,
                rating: 5
            }));
        ...
        it("returns 409 AlreadyExists if reviewing the same person twice for the same ride", async () => {
            const { driverCookie, passenger, rideId } = await setupCompletedRideWithPassenger();

            // First review
            await authenticatedRequest("/reviews", driverCookie, jsonRequest({
                rideId, subjectId: passenger.id, rating: 5
            }));

            // Second review
            const response = await authenticatedRequest("/reviews", driverCookie, jsonRequest({
                rideId, subjectId: passenger.id, rating: 3
            }));
    ...
    describe("Happy Paths", () => {
        it("successfully creates a review via POST /reviews and fetches via GET /reviews/users/:id", async () => {
            const { driverCookie, passengerCookie, driver, passenger, rideId } = await setupCompletedRideWithPassenger();

            // Driver reviews passenger
            const createRes = await authenticatedRequest("/reviews", driverCookie, jsonRequest({
                rideId, subjectId: passenger.id, rating: 5, comment: "Awesome!"
            }));
            ...
            await authenticatedRequest("/reviews", passengerCookie, jsonRequest({
                rideId, subjectId: driver.id, rating: 4, comment: "Good driving"
            }));

            // Fetch driver's public reviews
            const fetchRes = await authenticatedRequest(`/reviews/users/${driver.id}`, passengerCookie, { method: "GET" });
            expect(fetchRes.status).toBe(200);
            
            const fetchPayload = await fetchRes.json() as { averageRating: number; reviewCount: number; reviews: any[] };
        ...
        it("successfully fetches my authored reviews via GET /reviews/me/authored", async () => {
            const { passengerCookie, driver, rideId } = await setupCompletedRideWithPassenger();

            // Passenger reviews driver
            await authenticatedRequest("/reviews", passengerCookie, jsonRequest({
                rideId, subjectId: driver.id, rating: 5, comment: "Five stars"
            }));

            const authoredRes = await authenticatedRequest("/reviews/me/authored", passengerCookie, { method: "GET" });
