...
describe("BookingRoutes", () => {
    describe("Authorization & Onboarding Guards", () => {
        it("returns 401 UNAUTHORIZED for GET /bookings/me without a session", async () => {
            const response = await apiRequest("/bookings/me?timeframe=UPCOMING");
        ...
        it("returns 403 ONBOARDING_REQUIRED for GET /bookings/me with a non-onboarded user", async () => {
            ...
            const response = await authenticatedRequest("/bookings/me?timeframe=UPCOMING", cookie);
    ...
    describe("Passenger Routes", () => {
        it("creates a booking request via POST /bookings", async () => {
            ...
            const { user: passenger, cookie: passengerCookie } = await createSignedInUser();
            ...
            const response = await authenticatedRequest("/bookings", passengerCookie, jsonRequest(body));
        ...
        it("returns 409 SelfBookingNotAllowed if driver tries to book their own ride via POST /bookings", async () => {
            ...
            const response = await authenticatedRequest("/bookings", setup.driverCookie, jsonRequest(body));
        ...
        it("lists passenger bookings via GET /bookings/me", async () => {
            ...
            const response = await authenticatedRequest("/bookings/me?timeframe=UPCOMING", passengerCookie, { method: "GET" });
        ...
        it("cancels a passenger booking via PATCH /bookings/:id/cancel", async () => {
            ...
            const response = await authenticatedRequest(`/bookings/${bookingId}/cancel`, passengerCookie, {
                method: "PATCH",
        ...
        it("returns 403 when trying to cancel someone else's booking", async () => {
            ...
            const response = await authenticatedRequest(`/bookings/${bookingId}/cancel`, passenger2Cookie, {
                method: "PATCH",
                headers: { "content-type": "application/json" },
    ...
    describe("Driver Routes", () => {
        ...
        it("returns driver pending requests via GET /bookings/requests", async () => {
            ...
            const response = await authenticatedRequest("/bookings/requests", driverCookie, { method: "GET" });
        ...
        it("confirms a booking via PATCH /bookings/:id/confirm", async () => {
            ...
            const response = await authenticatedRequest(`/bookings/${bookingId}/confirm`, driverCookie, {
                method: "PATCH",
            });
        ...
        it("rejects a booking via PATCH /bookings/:id/reject", async () => {
            ...
            const response = await authenticatedRequest(`/bookings/${bookingId}/reject`, driverCookie, {
                method: "PATCH",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ reason: "Not enough space for luggage" })
            });
        ...
        it("driver cancels a confirmed booking via PATCH /bookings/:id/driver/cancel", async () => {
            const { driver, driverCookie, bookingId } = await setupPendingBooking();

            await BookingService.confirmBooking(bookingId, driver.id);

            const response = await authenticatedRequest(`/bookings/${bookingId}/driver/cancel`, driverCookie, {
                method: "PATCH",
                headers: { "content-type": "application/json" },
        ...
        it("returns 403 when a non-driver tries to confirm a booking", async () => {
            ...
            const response = await authenticatedRequest(`/bookings/${bookingId}/confirm`, strangerCookie, {
                method: "PATCH",
            });
