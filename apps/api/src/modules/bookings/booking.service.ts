 ...
 */
const createBookingRequest = async (payload: CreateBookingInput): Promise<string> => {
...
const cancelBookingByPassenger = async (bookingId: string, passengerId: string, reason?: string): Promise<string> => {
    return await BookingRepository.cancelBookingByPassenger(bookingId, passengerId, reason);
};
