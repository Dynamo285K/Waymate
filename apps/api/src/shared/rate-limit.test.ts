...
// Unique key per test so the module-level window map can't leak state between
// cases.
...
describe("checkRateLimit (in-memory fixed window)", () => {
    ...
    it("throws RateLimitError once `max` is exceeded", () => {
        const key = freshKey();
        for (let i = 0; i < 3; i++) checkRateLimit(key, 3, 60_000);
        expect(() => checkRateLimit(key, 3, 60_000)).toThrow(RateLimitError);
    });
    ...
    it("resets the counter after the window elapses", () => {
        ...
        expect(() => checkRateLimit(key, 1, 1_000)).toThrow(RateLimitError);
