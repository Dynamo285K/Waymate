...
export const rideStops = pgTable(
    ...
    (table) => [
        uniqueIndex("ride_stops_ride_id_stop_order_uq").on(table.rideId, table.stopOrder),
        ...
        check("ride_stops_address_len_chk", sql`char_length(${table.address}) BETWEEN 1 AND 255`),
        check("ride_stops_city_len_chk", sql`char_length(${table.city}) BETWEEN 1 AND 100`),
        check(
        ...
        check("ride_stops_country_code_chk", sql`${table.countryCode} IS NULL OR ${table.countryCode} ~ '^[A-Z]{3}$'`),
    ]
);