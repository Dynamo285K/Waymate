...
content = content.replace(
    /originCity: originCities\.name,\n\s*destinationCity: destCities\.name,/g,
...
content = content.replace(
    /city: citiesTable\.name,\n\s*countryCode: citiesTable\.countryCode,/g,
...
content = content.replace(
    /const originCities = aliasedTable\(citiesTable, "dash_origin_cities"\);\n\s*const destCities = aliasedTable\(citiesTable, "dash_dest_cities"\);\n/g,
    ""
);

// 8. Find any remaining innerJoin/leftJoin for cities and replace them
content = content.replace(
    /\.innerJoin\(originCities, eq\(originCities\.id, originStops\.cityId\)\)/g,
    ""
);
content = content.replace(
    /\.innerJoin\(destCities, eq\(destCities\.id, destStops\.cityId\)\)/g,
    ""
);
content = content.replace(
    /\.leftJoin\(originCities, eq\(originCities\.id, originStops\.cityId\)\)/g,
    ""
);
content = content.replace(
    /\.leftJoin\(destCities, eq\(destCities\.id, destStops\.cityId\)\)/g,
    ""
);
