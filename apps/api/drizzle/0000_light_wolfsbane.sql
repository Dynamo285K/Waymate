...
CREATE TABLE "cars" (
	...
	CONSTRAINT "cars_spz_len_chk" CHECK (char_length("cars"."spz") BETWEEN 1 AND 16),
	CONSTRAINT "cars_country_code_chk" CHECK ("cars"."country_code" ~ '^[A-Z]{3}$'),
	CONSTRAINT "cars_color_len_chk" CHECK ("cars"."color" IS NULL OR char_length("cars"."color") BETWEEN 1 AND 50)
);
--> statement-breakpoint
...
CREATE TABLE "ride_stops" (
	...
	CONSTRAINT "ride_stops_country_code_chk" CHECK ("ride_stops"."country_code" IS NULL OR "ride_stops"."country_code" ~ '^[A-Z]{3}$')
);
--> statement-breakpoint
CREATE TABLE "rides" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
...
CREATE INDEX "blocklist_created_at_idx" ON "blocklist" USING btree ("created_at");--> statement-breakpoint
